...
class SimulationRobobo(Robobo):
    ...
    def initialize_handles(self):
        self._RightMotor = self._vrep_get_object_handle(
            "Right_Motor{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._LeftMotor = self._vrep_get_object_handle(
            "Left_Motor{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        ...
        self._IrBackC = self._vrep_get_object_handle(
            "Ir_Back_C{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrFrontC = self._vrep_get_object_handle(
            "Ir_Front_C{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrFrontLL = self._vrep_get_object_handle(
            "Ir_Front_LL{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrFrontRR = self._vrep_get_object_handle(
            "Ir_Front_RR{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrBackL = self._vrep_get_object_handle(
            "Ir_Back_L{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrBackLFloor = self._vrep_get_object_handle(
            "Ir_Back_L_Floor{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrBackR = self._vrep_get_object_handle(
            "Ir_Back_R{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrBackRFloor = self._vrep_get_object_handle(
            "Ir_Back_R_Floor{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrFrontL = self._vrep_get_object_handle(
            "Ir_Front_L{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrFrontLFloor = self._vrep_get_object_handle(
            "Ir_Front_L_Floor{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrFrontR = self._vrep_get_object_handle(
            "Ir_Front_R{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._IrFrontRFloor = self._vrep_get_object_handle(
            "Ir_Front_R_Floor{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._TiltMotor = self._vrep_get_object_handle(
            "Tilt_Motor{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._PanMotor = self._vrep_get_object_handle(
            "Pan_Motor{}".format(self._value_number), vrep.simx_opmode_blocking
        )
        self._FrontalCamera = self._vrep_get_object_handle(
            "Frontal_Camera{}".format(self._value_number), vrep.simx_opmode_blocking
        )
    ...
    def wait_for_ping(self, timeout_seconds=120.0):
        ...
        print(
            "{} seconds passed with ping not coming online, you may expericence problems with the connection".format(
                timeout_seconds
            )
        )
        return False
    ...
    def _vrep_get_vision_sensor_image(self, camera_handle, opmode=vrep.simx_opmode_buffer, a=0):
        return vrep.unwrap_vrep(
            vrep.simxGetVisionSensorImage(self._clientID, camera_handle, a, opmode)
        )

    def _vrep_get_vision_sensor_image_ignore_error(
        self, camera_handle, opmode=vrep.simx_opmode_buffer, a=0
    ):
        try:
    ...
    def _vrep_set_joint_target_velocity(self, handle, speed, opmode):
        return vrep.unwrap_vrep(
            vrep.simxSetJointTargetVelocity(self._clientID, handle, speed, opmode)
        )

    def _vrep_set_joint_target_position(self, handle, position, opmode=vrep.simx_opmode_oneshot):
        return vrep.unwrap_vrep(
            vrep.simxSetJointTargetPosition(self._clientID, handle, position, opmode)
        )
    ...
    def move(self, left, right, millis=500):
        ...
        self._vrep_set_joint_target_velocity(
            self._LeftMotor, stopLeftVelocity, vrep.simx_opmode_oneshot
        )
        self._vrep_set_joint_target_velocity(
            self._RightMotor, stopRightVelocity, vrep.simx_opmode_oneshot
        )
        self.wait_for_ping()
    ...
    def read_irs(self):
        ...
        """
        (
            detectionStateIrFrontC,
            detectedPointIrFrontC,
            detectedObjectHandleIrFrontC,
            detectedSurfaceNormalVectorIrFrontC,
        ) = self._vrep_read_proximity_sensor(self._IrFrontC, vrep.simx_opmode_buffer)
        (
            detectionStateIrBackC,
            detectedPointIrIrBackC,
            detectedObjectHandleIrBackC,
            detectedSurfaceNormalVectorIrBackC,
        ) = self._vrep_read_proximity_sensor(self._IrBackC, vrep.simx_opmode_buffer)
        (
            detectionStateIrFrontLL,
            detectedPointIrFrontLL,
            detectedObjectHandleIrFrontLL,
            detectedSurfaceNormalVectorIrFrontLL,
        ) = self._vrep_read_proximity_sensor(self._IrFrontLL, vrep.simx_opmode_buffer)
        (
            detectionStateIrFrontRR,
            detectedPointIrFrontRR,
            detectedObjectHandleIrFrontRR,
            detectedSurfaceNormalVectorIrFrontRR,
        ) = self._vrep_read_proximity_sensor(self._IrFrontRR, vrep.simx_opmode_buffer)
        (
            detectionStateIrBackL,
            detectedPointIrBackL,
            detectedObjectHandleIrBackL,
            detectedSurfaceNormalVectorIrBackL,
        ) = self._vrep_read_proximity_sensor(self._IrBackL, vrep.simx_opmode_buffer)

        (
            detectionStateIrBackR,
            detectedPointIrBackR,
            detectedObjectHandleIrBackR,
            detectedSurfaceNormalVectorIrBackR,
        ) = self._vrep_read_proximity_sensor(self._IrBackR, vrep.simx_opmode_buffer)

        (
            detectionStateIrFrontR,
            detectedPointIrFrontR,
            detectedObjectHandleIrFrontR,
            detectedSurfaceNormalVectorIrFrontR,
        ) = self._vrep_read_proximity_sensor(self._IrFrontR, vrep.simx_opmode_buffer)

        (
            detectionStateIrFrontL,
            detectedPointIrFrontL,
            detectedObjectHandleIrFrontL,
            detectedSurfaceNormalVectorIrFrontL,
        ) = self._vrep_read_proximity_sensor(self._IrFrontL, vrep.simx_opmode_buffer)

        vect = [
            np.sqrt(
                detectedPointIrBackR[0] ** 2
                + detectedPointIrBackR[1] ** 2
                + detectedPointIrBackR[2] ** 2
            )
            if detectionStateIrBackR
            else False,
            np.sqrt(
                detectedPointIrIrBackC[0] ** 2
                + detectedPointIrIrBackC[1] ** 2
                + detectedPointIrIrBackC[2] ** 2
            )
            if detectionStateIrBackC
            else False,
            np.sqrt(
                detectedPointIrBackL[0] ** 2
                + detectedPointIrBackL[1] ** 2
                + detectedPointIrBackL[2] ** 2
            )
            if detectionStateIrBackL
            else False,
            np.sqrt(
                detectedPointIrFrontRR[0] ** 2
                + detectedPointIrFrontRR[1] ** 2
                + detectedPointIrFrontRR[2] ** 2
            )
            if detectionStateIrFrontRR
            else False,
            np.sqrt(
                detectedPointIrFrontR[0] ** 2
                + detectedPointIrFrontR[1] ** 2
                + detectedPointIrFrontR[2] ** 2
            )
            if detectionStateIrFrontR
            else False,
            np.sqrt(
                detectedPointIrFrontC[0] ** 2
                + detectedPointIrFrontC[1] ** 2
                + detectedPointIrFrontC[2] ** 2
            )
            if detectionStateIrFrontC
            else False,
            np.sqrt(
                detectedPointIrFrontL[0] ** 2
                + detectedPointIrFrontL[1] ** 2
                + detectedPointIrFrontL[2] ** 2
            )
            if detectionStateIrFrontL
            else False,
            np.sqrt(
                detectedPointIrFrontLL[0] ** 2
                + detectedPointIrFrontLL[1] ** 2
                + detectedPointIrFrontLL[2] ** 2
            )
            if detectionStateIrFrontLL
            else False,
        ]
    ...
    def pause_simulation(self):
        vrep.unwrap_vrep(vrep.simxPauseSimulation(self._clientID, vrep.simx_opmode_blocking))

    def play_simulation(self):
        vrep.unwrap_vrep(vrep.simxStartSimulation(self._clientID, vrep.simx_opmode_blocking))
        self.wait_for_ping()

    def stop_world(self):
        vrep.unwrap_vrep(vrep.simxStopSimulation(self._clientID, vrep.simx_opmode_blocking))
        self.wait_for_ping()
