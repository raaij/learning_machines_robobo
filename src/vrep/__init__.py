...
c_SetVisionSensorImage = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.c_int32, ct.POINTER(ct.c_byte), ct.c_int32, ct.c_ubyte, ct.c_int32
...
c_LoadModel = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.POINTER(ct.c_char), ct.c_ubyte, ct.POINTER(ct.c_int32), ct.c_int32
...
c_GetUIEventButton = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.c_int32, ct.POINTER(ct.c_int32), ct.POINTER(ct.c_int32), ct.c_int32
...
c_GetLastErrors = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.POINTER(ct.c_int32), ct.POINTER(ct.POINTER(ct.c_char)), ct.c_int32
...
c_GetObjectSelection = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.POINTER(ct.POINTER(ct.c_int32)), ct.POINTER(ct.c_int32), ct.c_int32
...
c_SetStringSignal = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.POINTER(ct.c_char), ct.POINTER(ct.c_ubyte), ct.c_int32, ct.c_int32
...
c_Start = ct.CFUNCTYPE(
    ct.c_int32, ct.POINTER(ct.c_char), ct.c_int32, ct.c_ubyte, ct.c_ubyte, ct.c_int32, ct.c_int32
)(("simxStart", libsimx))
...
c_TransferFile = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.POINTER(ct.c_char), ct.POINTER(ct.c_char), ct.c_int32, ct.c_int32
...
c_CreateDummy = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.c_float, ct.POINTER(ct.c_ubyte), ct.POINTER(ct.c_int32), ct.c_int32
...
c_GetObjectVelocity = ct.CFUNCTYPE(
    ct.c_int32, ct.c_int32, ct.c_int32, ct.POINTER(ct.c_float), ct.POINTER(ct.c_float), ct.c_int32
...
def simxReadForceSensor(clientID, forceSensorHandle, operationMode):
    ...
    ret = c_ReadForceSensor(
        clientID, forceSensorHandle, ct.byref(state), forceVector, torqueVector, operationMode
    )
    arr1 = []
    for i in range(3):
...
def simxGetObjectChild(clientID, parentObjectHandle, childIndex, operationMode):
    ...
    return (
        c_GetObjectChild(
            clientID, parentObjectHandle, childIndex, ct.byref(childObjectHandle), operationMode
        ),
...
def simxGetObjectParent(clientID, childObjectHandle, operationMode):
    ...
    return (
        c_GetObjectParent(clientID, childObjectHandle, ct.byref(parentObjectHandle), operationMode),
...
def simxLoadModel(clientID, modelPathAndName, options, operationMode):
    ...
    return (
        c_LoadModel(clientID, modelPathAndName, options, ct.byref(baseHandle), operationMode),
        baseHandle.value,
    )
...
def simxLoadUI(clientID, uiPathAndName, options, operationMode):
    ...
    ret = c_LoadUI(
        clientID, uiPathAndName, options, ct.byref(count), ct.byref(uiHandles), operationMode
    )
...
def simxGetUISlider(clientID, uiHandle, uiButtonID, operationMode):
    ...
    position = ct.c_int()
    return (
        c_GetUISlider(clientID, uiHandle, uiButtonID, ct.byref(position), operationMode),
        position.value,
    )
...
def simxGetUIButtonProperty(clientID, uiHandle, uiButtonID, operationMode):
    ...
    prop = ct.c_int()
    return (
        c_GetUIButtonProperty(clientID, uiHandle, uiButtonID, ct.byref(prop), operationMode),
        prop.value,
    )
...
def simxAuxiliaryConsoleOpen(
    clientID, title, maxLines, mode, position, size, textColor, backgroundColor, operationMode
):
    """
    Please have a look at the function description/documentation in the V-REP user manual
    """
...
def simxGetBooleanParameter(clientID, paramIdentifier, operationMode):
    ...
    return (
        c_GetBooleanParameter(clientID, paramIdentifier, ct.byref(paramValue), operationMode),
...
def simxGetIntegerParameter(clientID, paramIdentifier, operationMode):
    ...
    paramValue = ct.c_int()
    return (
        c_GetIntegerParameter(clientID, paramIdentifier, ct.byref(paramValue), operationMode),
        paramValue.value,
    )
...
def simxGetFloatingParameter(clientID, paramIdentifier, operationMode):
    ...
    return (
        c_GetFloatingParameter(clientID, paramIdentifier, ct.byref(paramValue), operationMode),
        paramValue.value,
    )
...
def simxGetCollisionHandle(clientID, collisionObjectName, operationMode):
    ...
    return (
        c_GetCollisionHandle(clientID, collisionObjectName, ct.byref(handle), operationMode),
        handle.value,
    )
...
def simxGetCollectionHandle(clientID, collectionName, operationMode):
    ...
    return (
        c_GetCollectionHandle(clientID, collectionName, ct.byref(handle), operationMode),
        handle.value,
    )
...
def simxGetDistanceHandle(clientID, distanceObjectName, operationMode):
    ...
    return (
        c_GetDistanceHandle(clientID, distanceObjectName, ct.byref(handle), operationMode),
        handle.value,
    )
...
def simxReadCollision(clientID, collisionObjectHandle, operationMode):
    ...
    return (
        c_ReadCollision(clientID, collisionObjectHandle, ct.byref(collisionState), operationMode),
...
def simxReadDistance(clientID, distanceObjectHandle, operationMode):
    ...
    return (
        c_ReadDistance(clientID, distanceObjectHandle, ct.byref(minimumDistance), operationMode),
...
def simxGetObjects(clientID, objectType, operationMode):
    ...
    ret = c_GetObjects(
        clientID, objectType, ct.byref(objectCount), ct.byref(objectHandles), operationMode
    )
    handles = []
    if ret == 0:
...
def simxDisplayDialog(
    clientID, titleText, mainText, dialogType, initialText, titleColors, dialogColors, operationMode
):
    """
    Please have a look at the function description/documentation in the V-REP user manual
    """
...
def simxGetStringSignal(clientID, signalName, operationMode):
    ...
    ret = c_GetStringSignal(
        clientID, signalName, ct.byref(signalValue), ct.byref(signalLength), operationMode
    )
