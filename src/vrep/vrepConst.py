...
sim_modelproperty_not_reset = (
    0x0080  # Model is not reset at simulation end. This flag is cleared at simulation end
)
...
sim_message_eventcallback_rmlposition = (
    0x12B  # the command simRMLPosition was called. The appropriate plugin should handle the call
)
sim_message_eventcallback_rmlvelocity = (
    0x12C  # the command simRMLVelocity was called. The appropriate plugin should handle the call
)
...
sim_drawing_quadpoints = (
    4  # 6 values per point (3 for quad position 3 for quad normal vector) (quad size in meters)
)
sim_drawing_discpoints = (
    5  # 6 values per point (3 for disc position 3 for disc normal vector) (disc size in meters)
)
sim_drawing_cubepoints = (
    6  # 6 values per point (3 for cube position 3 for cube normal vector) (cube size in meters)
)
...
sim_drawing_itemsizes = (
    0x00080  # +1 value per item (each item has its own size). Not for sim_drawing_triangles
)
...
sim_particle_ignoresgravity = (
    0x0100  # the particles ignore the effect of gravity. Not compatible with sim_particle_water
)
...
sim_particle_painttag = (
    0x10000  # The particles can be seen by vision sensors (sim_particle_invisible must not be set)
)
...
simx_headeroffset_version = (
    2  # 1 byte. Generated by the client or server. The version of the remote API software
)
...
simx_headeroffset_client_time = (
    7  # 1 simxInt. Client time stamp generated by the client (and sent back by the server)
)
simx_headeroffset_server_time = (
    11  # 1 simxInt. Generated by the server when a reply is generated. The server timestamp
)
...
simx_opmode_streaming = (
    0x020000  # sends command as one chunk. Command will be stored on the server and always executed
)
...
simx_return_timeout_flag = (
    0x000002  # command reply not received in time for opmode_oneshot_wait operation mode
)
